...
export default class SlideCoinToss extends Component {
  componentDidMount(){
    ...
    Array.from(document.querySelectorAll('.coin-toss-graph rect'))
      .forEach((bar, index) => {
        TweenMax.set(bar, { attr: positions[index]})
      })
    new TimelineMax()
      .staggerFrom('.coin-toss-graph rect', 1, { ease: Back.easeOut.config(1.2), scaleY: '0', transformOrigin: '50% 100%'}, 0.02)
      .from('.coin-toss-graph text', 0.3, { opacity: 0 }, '-=0.3')
  }
